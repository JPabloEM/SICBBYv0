<?php 
    ...
    function nav_info($data="")
    {
        $view_header = "Views/Template/nav_info.php";
        require_once ($view_header);
    }
    ...
    function footer_info($data="")
    {
        $view_footer = "Views/Template/footer_info.php";
        require_once ($view_footer);        
    }
    // function headerCorredor($data="")
    // {
    //     $view_header = "Views/Template/header_Corredor.php";
    //     require_once ($view_header);
    // }
    // function footerCorredor($data="")
    // {
    //     $view_footer = "Views/Template/footer_Corredor.php";
    //     require_once ($view_footer);        
    // }
        ...
        if(ENVIRONMENT == 1){
           ...
           //Create an instance; passing `true` enables exceptions
            ...
            try {
                ...
                $mail->Username   = 'jpespinozamarin02@gmail.com';          //SMTP username
                $mail->Password   = 'pabloelcrack123';                               //SMTP password
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
                $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

                //Recipients
                $mail->setFrom('jpespinozamarin02@gmail.com', 'Servidor Local');
    ...
    //aca se va a manejar lo de la documentacion
    function uploadImage(array $data, string $name){
        $url_temp = $data['tmp_name'];
        $destino    = 'Assets/documents/uploads1/'.$name;        
        $move = move_uploaded_file($url_temp, $destino);
        return $move;
    }

    function deleteFile(string $name){
        unlink('Assets/documents/uploads1/'.$name);
    }

///DOC PUBLICA
    function uploadImageP(array $data, string $name){
        $url_temp = $data['tmp_name'];
        $destino    = 'Assets/documents/uploads2/'.$name;        
        $move = move_uploaded_file($url_temp, $destino);
        return $move;
    }

    function deleteFileP(string $name){
        unlink('Assets/documents/uploads2/'.$name);
    }
